...
if doc_type == "Articles of Incorporation":
    ...
    if st.button("Forge Articles of Incorporation"):
        data = {
            'corp_name': corp_name,
            'corp_address': corp_address.replace('\n', '<br>'),
            'reg_agent': reg_agent,
            'shares': f"{shares:,}"
        }
    ...
    if st.button("Forge Bank Statements"):
        st.info("Note: This will generate and download the *most recent* of the 6 statements.")
        # A future version could zip all 6 files.
